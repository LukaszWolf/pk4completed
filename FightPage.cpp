...
FightPage::FightPage(Game& game) : Page(game) {
    content_area = new ContentArea(1920, 1080, game, "Textures/fight_background.png", 0, 0);
    player_img.setSize({ 600.f, 600.f });
    player_img.setPosition(180.f, 180.f);

    enemy_img.setSize({ 600.f, 600.f });
    enemy_img.setPosition(1120.f, 180.f);
    ...
    player_hp_text.setCharacterSize(20);
    player_hp_text.setPosition(300.f, 830.f);
    player_hp_text.setString("0");

    enemy_hp_text.setFont(font);
    enemy_hp_text.setFillColor(sf::Color::White);
    enemy_hp_text.setCharacterSize(20);
    enemy_hp_text.setPosition(1200.f, 830.f);
    enemy_hp_text.setString("0");

    player_hp_bar = new HPBar(200.f, 800.f, 600.f, 100.f, sf::Color::Red);
    enemy_hp_bar = new HPBar(1100.f, 800.f, 600.f, 100.f, sf::Color::Red);
    player_current_hp = 0;
    enemy_current_hp = 0;
    player_max_hp = 0;
    enemy_max_hp = 0;
...
void FightPage::draw(sf::RenderWindow& window) {
    ...
    if (fight_initialized && enemy_hp_bar && enemy_hp_bar->getDisplayedHP() <= 0.f) {
        ...
        fight_initialized = false;  // <== to dodaj
        game_ref.setFightActiveFlag(false); // jeœli masz taki mechanizm
...
void FightPage::setPlayerAndEnemyImg() {
    ...
    enemy_img_texture.loadFromFile(loggedInUser->getActiveEnemy()->getImgName()); // lub loggedInUser->getActiveEnemy()->getImgName()
...
void FightPage::Fight(sf::RenderWindow& window) {
    // niewykorzystane
}
